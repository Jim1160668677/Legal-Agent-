...
`npm test`（`vitest run`）实际输出：

```
 Test Files  114 passed (114)
      Tests  1292 passed | 25 skipped | 0 failed
   Duration  ~323s
```
...
> 跳过全部为环境依赖，非代码缺陷：
> - **20 个 agnes 集成测试**：AGNES key 已配置（`sk-EGw4...`），但 `apihub.agnes-ai.com` 在当前环境不可达（curl 8s 超时）；网络通畅时自动运行。
> - **5 个 zhipu 集成测试**：调用真实免费模型 `glm-4.7-flash`，遇速率限制(429)或超时(30s)时优雅跳过；模型未过载时正常执行并验证全链路。
